...
import './ResTitle.css'; // Импортируем новый файл стилей
...
function ResTitle() {
  ...
  useEffect(() => {
    ...
    const fetchDocumentId = async () => {
        try {
          const response = await fetch(`https://localhost:7136/api/ChapterContent/nntable/${nnTableId}`);
          if (!response.ok) {
            throw new Error('Ошибка при загрузке ID альбома');
          }
          const chapterIds = await response.json();
          if (chapterIds.length > 0) {
            fetchDocId(chapterIds[0]);
            fetchChapter(chapterIds[0]);
          }
        } catch (error) {
    ...
    const fetchDocId = async (chapterId) => {
      try {
